...
class AppFABMenuController: FABMenuController {
    ...
    open override func prepare() {
        ...
        setupFABButton()
        setupNotesFABMenuItem()
        setupRemindersFABMenuItem()
        setupFABMenu()
    }

    // MARK: - UI

    fileprivate func setupFABButton() {
        fabButton = FABButton(image: Icon.cm.add, tintColor: .white)
        fabButton.pulseColor = .white
        fabButton.backgroundColor = Color.red.base
    }

    fileprivate func setupNotesFABMenuItem() {
        ...
        notesFABMenuItem.fabButton.addTarget(self,
                                             action: #selector(handleNotesFABMenuItem(button:)),
                                             for: .touchUpInside)
    }

    fileprivate func setupRemindersFABMenuItem() {
        ...
        remindersFABMenuItem.fabButton.addTarget(self,
                                                 action: #selector(handleRemindersFABMenuItem(button:)),
                                                 for: .touchUpInside)
    }

    fileprivate func setupFABMenu() {
